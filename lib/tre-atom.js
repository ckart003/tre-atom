    ...
    activate(state) {
        ...
        var promise = new Promise(
            function(resolve, reject) {
                t.get("/1/members/"+trello_config.username+"/boards", function(err, data) {
                    if (err) throw (err);
                    resolve(data);
                })
            }
    ...
    return function() {
     ...
     //    document.getElementById('secondMenu').style.visibility = "visible";
        ...
        getLists(board_id).then(function(res) {
            ...
            var header = document.createElement('h3');
            header.innerHTML = "Where's your story?!"
            menu.appendChild(header);
    ...
    return function() {
        console.log(comment.value);
             ...
             goBack.addEventListener('click',function(){
                  ...
                  document.getElementById('fourthMenu').style.display = "none";
             })
             document.getElementById('fourthMenu').appendChild(goBack);
    }
}
...
function lastStage(card_id) {
    return function() {
        ...
        var header = document.createElement('h3');
        header.innerHTML = "Add Comment and Move Card!"
        menu.appendChild(header);
...
function getCards(lists_id) {
    ...
    return function() {
        ...
        cardPromise.then(function(res) {
           ...
           var header = document.createElement('h3');
           header.innerHTML = "Pick Your Card!"
           menu.appendChild(header);
            var table = document.createElement('table');
            for (var i = 0; i < res.length; i++) {
                var inputID = document.createElement('button');
